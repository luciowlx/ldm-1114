...
- [Feat/ProjectManagement/Filters] 项目管理筛选位置调整：状态筛选移至顶部工具栏，负责人筛选移至列表列头。
  - 涉及文件：`src/App.tsx`
  - 说明/验证：顶部工具栏显示状态筛选下拉；列表“负责人”列头显示筛选图标，点击弹出负责人选择；筛选联动正常。

- [Tweak/Dashboard/Stats] 首页数据统计调整：将“来源”改为“今日新增”。
  - 涉及文件：`src/components/Dashboard.tsx`
  - 说明/验证：首页“数据统计”卡片显示“今日新增”及其数值。

### 2025-11-26

- [Chore/Header/Version] 头像菜单版本标识调整为 `v1.0`（移除“任务引擎”字样）。
  - 涉及文件：`src/components/Header.tsx`、`说明文档.md`、`CHANGELOG.md`
  - 说明/验证：`npm run build` 构建通过；本地预览头像菜单显示为 `v1.0`；无控制台报错。

- [Tweak/DataManagement/EditModal] 编辑数据集弹窗移除“数据源”选择（仅保留名称、描述、标签等字段）。
  - 涉及文件：`src/components/DataManagement.tsx`
  - 说明/验证：编辑弹窗不再出现“数据源”选择；保存编辑正常；页面与终端无报错。

- [Docs/Sync] 同步文档版本描述为 `v1.0`。
  - 涉及文件：`说明文档.md`、`CHANGELOG.md`
  - 说明/验证：执行 `npm run check:changelog` 通过；文档与展示一致。
